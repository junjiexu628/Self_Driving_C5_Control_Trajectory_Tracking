...
int main ()
{
  ...
  **/
  ...
  //seventh try, adjust steer
  //pid_steer.Init(0.1,0.005,0.3,-1.2,1.2);
  //pid_throttle.Init(0.2,0.015,0.1,-1.0,1.0);
  
  //seventh try, adjust steer
  //pid_steer.Init(0.3,0.05,0.1,-1.2,1.2);
  //pid_throttle.Init(0.25,0.05,0.1,-1.0,1.0);
  
    //eighth try, adjust steer
  //pid_steer.Init(0.1,0.05,0.3,-1.2,1.2);
  //pid_throttle.Init(0.25,0.05,0.1,-1.0,1.0);
  
      //ninth try, adjust steer
  //pid_steer.Init(0.2,0.05,0.3,-1.2,1.2);
  //pid_throttle.Init(0.25,0.05,0.1,-1.0,1.0);
  
        //tenth try, adjust steer
  //pid_steer.Init(0.32,0.05,0.1,-1.2,1.2);
  //pid_throttle.Init(0.25,0.05,0.1,-1.0,1.0);
  
          //11th try, adjust steer
  //pid_steer.Init(0.2,0.03,0.02,-1.2,1.2);
  //pid_throttle.Init(0.25,0.05,0.1,-1.0,1.0);
  
            //12th try, adjust steer
  pid_steer.Init(0.25,0.05,0.5,-1.2,1.2);
  pid_throttle.Init(0.25,0.05,0.1,-1.0,1.0);
...
//           // Update the delta time with the previous command
          ...
          **/
          ...
          error_throttle = velocity - v_points[close_id];
